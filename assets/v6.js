...
(function(){
  ...
  function startOnView(elId, maker){
    var el=document.getElementById(elId); if(!el) return;
    var term=maker(el);
    if(reduce){ term.run(); return; }
    var obs=new IntersectionObserver(function(es){es.forEach(function(e){ if(e.isIntersecting){ term.run(); obs.disconnect(); }});},{threshold:.3});
    obs.observe(el.closest('.term')||el);
  }
  startOnView('termBody', function(el){ return Terminal(el, heroFrames, {loop:true, pauseEnd:3600, speed:20}); });
  startOnView('callBody', function(el){ return buildCodegen(el); });
